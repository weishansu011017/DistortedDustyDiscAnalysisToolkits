...
Depth–first traversal of a Linear BVH **using an explicit software stack**.
The traversal visits internal nodes and leaf nodes in Morton order; leaf
indices intersecting the spherical query centered at `point` with radius
`radius` are written into `pool`.  
The `stack` buffer is reused for DFS and prevents allocations during queries.

# Parameters
- `pool::AbstractVector{Int}`  
  Output buffer that receives leaf indices whose leaf–AABBs intersect the
  spherical query. The valid prefix is returned via a `NeighborSelection`
  handle.

- `stack::AbstractVector{Int}`  
  Scratch stack used for explicit depth–first traversal. Must be large enough
  to hold the worst–case number of simultaneously active internal nodes.

- `lbvh::LinearBVH{D,T}`  
  Linear Bounding Volume Hierarchy constructed from Morton–sorted primitives.

- `point::NTuple{D,T}`  
  Query position in the coordinate system of the BVH.

- `radius::Real`  
  Query radius; promoted to the BVH’s floating-point type if necessary.

# Returns
- `NeighborSelection`  
  View of `pool` containing the number of emitted neighbors and the closest
  leaf index (minimum squared distance).

# Notes
- When `lbvh.root == 0`, traversal degenerates into scanning all leaves.
- Explicit–stack DFS matches CPU behavior and is predictable but requires
  per-query scratch space, making this version less suitable for GPU kernels.
"""
...
@inline function LBVH_query!(pool::VI, stack::VI, LBVH::LinearBVH{D, T},
    ...
    @inline dist2_to_node(i::Int) = _dist2_to_node_aabb(node_min, node_max, point, i)
    @inline leaf_dist2(i::Int) = _dist2_to_leaf_aabb(leaf_min, leaf_max, point, i)
