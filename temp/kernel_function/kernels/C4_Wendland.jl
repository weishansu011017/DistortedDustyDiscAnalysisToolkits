...
parentkernel(::_dC4_Wendland) = C4_Wendland()
...
````
"""
@inline KernelFunctionValid(::C4_Wendland, ::Type{T}) where {T<:AbstractFloat} = T(2.0)
...
```
"""
@inline KernelFunctionnorm(::C4_Wendland, ::Val{1}, ::Type{T}) where {T<:AbstractFloat} = T(3) / T(4)
@inline KernelFunctionnorm(::C4_Wendland, ::Val{2}, ::Type{T}) where {T<:AbstractFloat} = T(9) / (T(4) * T(π))
@inline KernelFunctionnorm(::C4_Wendland, ::Val{3}, ::Type{T}) where {T<:AbstractFloat} = T(495) / (T(256) * T(π))
...
```
"""
@inline KernelFunctionDiff(::C4_Wendland, q :: T) where {T<:AbstractFloat} = _dC4_Wendland()(q)
...
```
"""
@inline KernelFunctionNneigh(::C4_Wendland) = 137