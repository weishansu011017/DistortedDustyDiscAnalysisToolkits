...
parentkernel(::_dM6_spline) = M6_spline()
...
````
"""
@inline KernelFunctionValid(::M6_spline, ::Type{T}) where {T<:AbstractFloat} = T(3.0)
...
```
"""
@inline KernelFunctionnorm(::M6_spline, ::Val{1}, ::Type{T}) where {T<:AbstractFloat} = T(1) / T(120)
@inline KernelFunctionnorm(::M6_spline, ::Val{2}, ::Type{T}) where {T<:AbstractFloat} = T(7) / (T(478) * T(π))
@inline KernelFunctionnorm(::M6_spline, ::Val{3}, ::Type{T}) where {T<:AbstractFloat} = T(1) / (T(120) * T(π))
...
```
"""
@inline KernelFunctionDiff(::M6_spline, q :: T) where {T<:AbstractFloat} = _dM6_spline()(q)
...
```
"""
@inline KernelFunctionNneigh(::M6_spline) = 112