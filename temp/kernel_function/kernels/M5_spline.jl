...
parentkernel(::_dM5_spline) = M5_spline()
...
````
"""
@inline KernelFunctionValid(::M5_spline, ::Type{T}) where {T<:AbstractFloat} = T(2.5)
...
```
"""
@inline KernelFunctionnorm(::M5_spline, ::Val{1}, ::Type{T}) where {T<:AbstractFloat} = T(1) / T(24)
@inline KernelFunctionnorm(::M5_spline, ::Val{2}, ::Type{T}) where {T<:AbstractFloat} = T(96) / (T(1199) * T(π))
@inline KernelFunctionnorm(::M5_spline, ::Val{3}, ::Type{T}) where {T<:AbstractFloat} = T(0.05) / T(π)
...
```
"""
@inline KernelFunctionDiff(::M5_spline, q :: T) where {T<:AbstractFloat} = _dM5_spline()(q)
...
```
"""
@inline KernelFunctionNneigh(::M5_spline) = 113