...
parentkernel(::_dC2_Wendland) = C2_Wendland()
...
````
"""
@inline KernelFunctionValid(::C2_Wendland, ::Type{T}) where {T<:AbstractFloat} = T(2.0)
...
```
"""
@inline KernelFunctionnorm(::C2_Wendland, ::Val{1}, ::Type{T}) where {T<:AbstractFloat} = T(5) / T(8)
@inline KernelFunctionnorm(::C2_Wendland, ::Val{2}, ::Type{T}) where {T<:AbstractFloat} = T(7) / (T(4) * T(π))
@inline KernelFunctionnorm(::C2_Wendland, ::Val{3}, ::Type{T}) where {T<:AbstractFloat} = T(21) / (T(16) * T(π))
...
```
"""
@inline KernelFunctionDiff(::C2_Wendland, q :: T) where {T<:AbstractFloat} = _dC2_Wendland()(q)
...
```
"""
@inline KernelFunctionNneigh(::C2_Wendland) = 92