...
parentkernel(::_dM4_spline) = M4_spline()
...
````
"""
@inline KernelFunctionValid(::M4_spline, ::Type{T}) where {T<:AbstractFloat} = T(2.0)
...
```
"""
@inline KernelFunctionnorm(::M4_spline, ::Val{1}, ::Type{T}) where {T<:AbstractFloat} = T(4) / T(3)
@inline KernelFunctionnorm(::M4_spline, ::Val{2}, ::Type{T}) where {T<:AbstractFloat} = T(10) / (T(7) * T(π))
@inline KernelFunctionnorm(::M4_spline, ::Val{3}, ::Type{T}) where {T<:AbstractFloat} = T(1) / T(π)
...
```
"""
@inline KernelFunctionDiff(::M4_spline, q :: T) where {T<:AbstractFloat} = _dM4_spline()(q)
...
```
"""
@inline KernelFunctionNneigh(::M4_spline) = 57