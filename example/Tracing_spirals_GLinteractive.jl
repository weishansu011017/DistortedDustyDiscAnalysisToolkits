...
"""
Tracing N-spirals from a Face-on PhantomRevealer disc result with interactive interface based on GLMakie (OpenGL required)
    by Wei-Shan Su,
    June 17, 2025
"""
...
function spirals_tracer(file :: String; Faxg :: FigureAxes, Faxd :: FigureAxes,  index :: Int64, result :: pitch_result)
    # ------------------------------BRIEF INTRODUCTION------------------------------
    ...
    width_pixel_range :: Tuple{Float64, Float64} = (5.0, 22.0)  # The range of width of ridge IN PIXEL.
    ...
    λ_overlap :: Float64 = 1.2                                  # Weight of inter-arm overlap penalty.
    # -----------------------------------------------------------------------------
    index_g = 2
    index_d = 3
    @info "-------------------------------------------------------"
    ...
    if !isnothing(Faxg)
        Faxg = Faceon_polar_plot!(data,index_g,Fax=Faxg, colormap="binary")
        draw_Fig!(Faxg)
    end
    if !isnothing(Faxd)
        Faxd = Faceon_polar_plot!(data,index_d,Fax=Faxd, colormap="binary")
        draw_Fig!(Faxd)
    end

    if !isnothing(Faxg) || !isnothing(Faxd)
        print("Press Enter: ")
        readline()
    end
    @info "-------------------------------------------------------"
...
function draw_pitch2t_diagram(result :: pitch_result)
    ...
    set_annotation!(Fax, (1,1), L"St$\sim$15", halign=:right)
    save_Fig!(Fax, "Spiral_PitchAngle.png")
end
...
function main()
    ...
    if visualization
        test_data = Read_HDF5(files[1], true)
        transfer_cgs!(test_data)
        Faxg = Faceon_polar_plot!(test_data, 2, Fax = Faxg)
        Faxd = Faceon_polar_plot!(test_data, 3, Fax = Faxd)
    end

    for (i,file) in enumerate(files)
        @info "File: $file"
        spirals_tracer(file, index = i, result = result, Faxg = Faxg, Faxd = Faxd)
    end
