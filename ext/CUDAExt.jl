...
# Overview
`Hello_CUDA()` is intentionally simple: it serves one purpose—to prove that
...
julia> PhantomRevealer.Hello_CUDA()
...
the main module with function `Hello_CUDA`` end to stay on her good side.
...
"""
function PhantomRevealer.Hello_CUDA()
    @info "Hello from CUDAExt!"
    CUDA.versioninfo()
end
