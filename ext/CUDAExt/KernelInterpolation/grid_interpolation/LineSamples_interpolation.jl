@inline function _line_samples_interpolation_kernel!(grids :: NTuple{N, LineSamples{3}}, input :: ITPINPUT, catalog_consice :: InterpolationCatalogConcise{N, 0, 0, 0}, LBVH :: LinearBVH, ::Type{itpScatter}) where {N, TF <: AbstractFloat, ITPINPUT <: InterpolationInput{TF}}
    ...
    while i <= npoints
        ...
        scalars :: NTuple{N, TF} = PhantomRevealer.KernelInterpolation._line_integrated_quantities_interpolate_kernel(input, origin, direction, LBVH, scalar_slots, scalar_snormalization, itpScatter)
