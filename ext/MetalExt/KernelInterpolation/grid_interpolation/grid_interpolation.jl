...
@inline function _general_grid_interpolation_kernel!(:: MetalComputeBackend, grids :: NTuple{L, GeneralGrid{D}}, input :: ITPINPUT, catalog_consice :: InterpolationCatalogConcise{N, G, Div, C}, LBVH :: LinearBVH, itp_strategy::Type{ITPSTRATEGY} = itpSymmetric) where {D, N, G, Div, C, L, TF <: Float32, ITPINPUT <: InterpolationInput{TF}, ITPSTRATEGY <: AbstractInterpolationStrategy}
    # Use a 0-based global index and derive a positive stride
...
function PhantomRevealer.GeneralGrid_interpolation(backend :: MetalComputeBackend, grid_template::GeneralGrid{D}, input::ITPINPUT, catalog::InterpolationCatalog{N, G, Div, C, L}, itp_strategy::Type{ITPSTRATEGY} = itpSymmetric) where {D, N, G, Div, C, L, T <: AbstractFloat, ITPINPUT <: InterpolationInput{T}, ITPSTRATEGY <: AbstractInterpolationStrategy}
    ...
    @time begin
    @metal threads=(256,) groups=(cld(npoints, 256)) _general_grid_interpolation_kernel!(backend, grids_Mtl, input_Mtl, catalog_consice, LBVH_Mtl, itp_strategy)
    Metal.synchronize()
    end
    @info"End interpolation."
    @info "Copying interpolated grids back to host memory..."
    @time begin
    grids_result = ntuple(i -> to_HostVector(grids_Mtl[i]), Val(L))
    end
