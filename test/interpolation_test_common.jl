...
function brute_gradient_density(input::InterpolationInput{T}, ref::NTuple{3,T}, ha::T, strategy) where {T}
    Kvalid = KernelFunctionValid(typeof(kern), T)
    ∇ρf = zeros(T, 3)
    ∇ρb = zeros(T, 3)
    ρ = zero(T)
...
function brute_gradient_quantity(input::InterpolationInput{T}, ref::NTuple{3,T}, ha::T, col::Int, strategy) where {T}
    Kvalid = KernelFunctionValid(typeof(kern), T)
    ∇Af = zeros(T, 3)
    ∇Ab = zeros(T, 3)
    A = zero(T)
...
function brute_divergence(input::InterpolationInput{T}, ref::NTuple{3,T}, ha::T, cols::NTuple{3,Int}, strategy) where {T}
    Kvalid = KernelFunctionValid(typeof(kern), T)
    ∇Af = zero(T)
    ∇Axb = zero(T)
    ∇Ayb = zero(T)
...
function brute_curl(input::InterpolationInput{T}, ref::NTuple{3,T}, ha::T, cols::NTuple{3,Int}, strategy) where {T}
    Kvalid = KernelFunctionValid(typeof(kern), T)
    ∇Axf = zero(T)
    ∇Ayf = zero(T)
    ∇Azf = zero(T)
...
function brute_line_integrated_quantity(input::InterpolationInput{T}, origin::NTuple{3,T}, direction::NTuple{3,T}, ha::T, col::Int, strategy) where {T}
    Kvalid = KernelFunctionValid(typeof(kern), T)
    numer = zero(T)
    denom = zero(T)
    @inbounds for i in 1:input.Npart
