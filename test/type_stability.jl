...
using PhantomRevealer.TinyEigvals: tiny_eigvals!
...
function make_type_stability_input_3d()
    ...
    input = InterpolationInput{Float64, Vector{Float64}, typeof(kern), 3}(
        length(x), kern, x, y, z, m, h, ρ, (q1, q2, q3),
    )
...
# ── 6. Tiny fixed-size eigensolver ──────────────────────────────────── #

@testset "Type stability — tiny eigensolver" begin
