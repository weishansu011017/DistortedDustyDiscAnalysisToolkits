...
function PhantomRevealer.ClassicalSIGrowthRateInput(St::T, ε::T) where {T<:AbstractFloat}
    vxlcs = uxlcₛ(ε, St)
    vylcs = uylcₛ(ε, St)
    wxlcs = wxlcₛ(ε, St)
    wylcs = wylcₛ(ε, St)
    ρg    = 1.0
    ρd    = ρ̃d(ρg, ε)
    return PhantomRevealer.ClassicalSIGrowthRateInput(St, ρg, ρd, vxlcs, vylcs, wxlcs, wylcs)
end
