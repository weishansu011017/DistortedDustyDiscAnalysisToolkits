...
pdf_list = read_gizmo("./test/testinput/snap_375.hdf5", Full_data=false);
datag = pdf_list[1];
# add_rho!(datag);
input, catalog = build_input(datag, MassFromColumn(:m), scalars=Symbol[:rho, :vx, :vy, :vz], gradients = [:rho] ,divergences=[:v], curls=[:v],  smoothed_kernel=M6_spline);
grid = GeneralGrid(datag.dfdata.x[1:N], datag.dfdata.y[1:N], datag.dfdata.z[1:N]);
LBVH = LinearBVH!(input, Val(3), CodeType=UInt64);
catalog_concise = to_concise_catalog(catalog);

@info "----------------------------- DOING GRID INTERPOLATION TESTS -----------------------------"
griditpresult = GeneralGrid_interpolation(CUDAComputeBackend(), grid, input, catalog, itpScatter);
println()
griditpresult = GeneralGrid_interpolation(CUDAComputeBackend(), grid, input, catalog, itpScatter);
println()
griditpresult = GeneralGrid_interpolation(CUDAComputeBackend(), grid, input, catalog, itpScatter);
...
# @btime KernelInterpolation._general_quantity_interpolate_kernel($input, (40.0,40.0,0.0), 2.0, $LBVH, $catalog_concise,itpScatter);